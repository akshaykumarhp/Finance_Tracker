...
export default function HouseSwitcher({
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        onClick={() => setOpen((v) => !v)}
        className="flex items-center gap-2 rounded-xl border border-ink-200 bg-white px-3 py-2 text-sm font-semibold text-ink-700 shadow-sm transition hover:border-ink-300 hover:bg-ink-50 dark:border-ink-600 dark:bg-ink-800 dark:text-ink-200 dark:hover:border-ink-500 dark:hover:bg-ink-700"
      >
      ...
      {open && (
        <div className="absolute left-0 z-30 mt-2 w-60 overflow-hidden rounded-2xl border border-ink-200/70 bg-white p-1.5 shadow-card-hover dark:border-ink-700 dark:bg-ink-800">
          ...
          {houses.map((h) => {
            ...
            return (
              ...
              >
                ...
                {on && <Check className="h-4 w-4 text-brand-600 dark:text-brand-300" />}
              </button>
            );
          })}
        </div>
